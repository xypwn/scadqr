...
module qr(message, error_correction="M", width=100, height=100, thickness=1, center=false, mask_pattern=0, encoding="UTF-8") {
    ...
    place_data_bits(bits, mask_pattern, size);
}
...
//
module place_data_bits(bits, mask_pattern, size, pos=undef, i=0) {
    p = next_module_position(pos, size);
    x = p[0];
    y = p[1];
    if(p != undef) {
        ...
        place_data_bits(bits, mask_pattern, size, p, i+1);
    }
}
